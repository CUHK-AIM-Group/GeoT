...
@DataTransforms.register_module()
class PointCloudScaling_s(object):
    def __init__(self,
                 scale_s=[2. / 3, 3. / 2],
                 anisotropic=True,
                 scale_xyz=[True, True, True],
                 mirror=[0, 0, 0],  # the possibility of mirroring. set to a negative value to not mirror
                 **kwargs):
        self.scale_min, self.scale_max = np.array(scale_s).astype(np.float32)
        self.anisotropic = anisotropic
        self.scale_xyz = scale_xyz
        self.mirror = torch.from_numpy(np.array(mirror))
        self.use_mirroring = torch.sum(torch.tensor(self.mirror)>0) != 0
...
@DataTransforms.register_module()
class PointCloudTranslation_s(object):
    def __init__(self, shift_s=[0.2, 0.2, 0.], **kwargs):
        self.shift = torch.from_numpy(np.array(shift_s)).to(torch.float32)
...
@DataTransforms.register_module() 
class PointCloudScaleAndTranslate_s(object):
    def __init__(self, scale_s=[2. / 3, 3. / 2], scale_xyz=[True, True, True],  # ratio for xyz dimenions
                 anisotropic=True,
                 shift_s=[0.2, 0.2, 0.2],
                 mirror=[0, 0, 0],  # the possibility of mirroring. set to a negative value to not mirror
                 **kwargs):
...
@DataTransforms.register_module()
class PointCloudJitter_s(object):
    def __init__(self, jitter_sigma_s=0.01, jitter_clip_s=0.05, **kwargs):
        self.noise_std = jitter_sigma_s
        self.noise_clip = jitter_clip_s
...
@DataTransforms.register_module()
class PointCloudRotation_s(object):
    def __init__(self, angle_s=[0, 0, 0], **kwargs):
        self.angle = np.array(angle_s) * np.pi
