...
def cal_model_parm_nums_separate(model):
    total = sum([param.nelement() for param in model.parameters()])
    param_encoder = sum([param.nelement() for name, param in model.named_parameters() if 'encoder' in name])
    param_generator = sum([param.nelement() for name, param in model.named_parameters() if 'generator' in name])
    param_decoder = sum([param.nelement() for name, param in model.named_parameters() if 'decoder' in name])
    return total, param_encoder, param_generator, param_decoder
...
def save_checkpoint(cfg, model, epoch, optimizer=None, scheduler=None,
    ...
    if 'clean' in post_fix:
        clean_state_dict = {}
        model_state_dict = model.module.state_dict() if hasattr(model, 'module') else model.state_dict()
        for name in model_state_dict.keys():
            if name[:len("encoder.")] == "encoder.":
                clean_state_dict[name[len("encoder."):]] = model_state_dict[name]
        save_dict = {
...
def resume_checkpoint(config, model, optimizer=None, scheduler=None, pretrained_path=None, printer=logging.info):
    ...
    printer("=> loaded successfully '{}' (epoch {}, best val {})".format(pretrained_path, checkpoint['epoch'], str(best_metrics)))
    del checkpoint
    torch.cuda.empty_cache()

    return config.start_epoch, best_metrics


def load_checkpoint(model, pretrained_path, prefix=None, module=None, return_pretrained_keys=False):
    if not os.path.exists(pretrained_path):
    ...
    #---------------------------------------
    # base_ckpt = {k.replace("encoder.", ""): v for k, v in ckpt_state_dict.items()}
    #---------------------------------------
    if module is not None:
        base_ckpt = {k:v for k, v in base_ckpt.items() if module in k}
    
    if prefix is not None:
        base_ckpt = {prefix+k:v for k, v in base_ckpt.items()}
    ...
    metrics = {}
    for key in state_dict.keys():
        is_metric_key = sum([item in key for item in ['metric', 'miou', 'dsc', 'acc', 'test', 'val']]) > 0
        if is_metric_key:
            metrics[key] = state_dict[key]
    logging.info(f'ckpts @ {epoch} epoch( {metrics} )')
    if return_pretrained_keys:
        return state_dict['model'].keys()
    else:
