...
class LocalGrouper(nn.Module):
    ...
    def forward(self, xyz, points, idx_fps_list):
        B, N, C = xyz.shape
        S = self.groups
        ...
        return new_xyz, new_points, idx_fps_list
...
class PointMLPPartSegmentor(nn.Module):
    def __init__(self, num_classes=17, points=16000, embed_dim=64, groups=1, res_expansion=1.0,
        ...
        super(PointMLPPartSegmentor, self).__init__()
        ...
        # )
        self.cls_map = nn.Sequential(
            ConvBNReLU1D(2, cls_dim, bias=bias, activation=activation),
            ConvBNReLU1D(cls_dim, cls_dim, bias=bias, activation=activation)
        )
    ...
    def forward(self, p, x=None, cls_label=None):
        if isinstance(p, dict):
            p, x, cls_label = p['pos'], p.get('x', None), p['cls']
        if x is None:
        ...
        for i in range(self.stages):
            # Give xyz[b, p, 3] and fea[b, p, d], return new_xyz[b, g, 3] and new_fea[b, g, k, d]
            xyz, x, idx_fps = self.local_grouper_list[i](xyz, x.permute(0, 2, 1), idx_fps)  # [b,g,3]  [b,g,k,d]
        ...
        # cls_label_onehot = F.one_hot(cls_label, 16).transpose(1, 2).float()
        cls_label_onehot = F.one_hot(cls_label, 2).transpose(1, 2).float()
        cls_token = self.cls_map(cls_label_onehot)  # [b, cls_dim, 1]
        x = torch.cat([x, global_context.repeat([1, 1, x.shape[-1]]), cls_token.repeat([1, 1, x.shape[-1]])], dim=1)
        x = self.classifier(x)
        x = F.log_softmax(x, dim=1)
        return x, f_enc, f_dec, idx_fps
